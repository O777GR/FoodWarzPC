...
def render_water_tab() -> None:
    ...
    with st.sidebar:
        ...
        water_goal = st.number_input("Цель (мл/день)", value=DEFAULT_WATER_GOAL_ML, step=100)
        portion_size = st.number_input("Размер порции (мл)", value=DEFAULT_WATER_PORTION_ML, step=50)
    ...
    if col1.button(f"🥛 {portion_size} мл", use_container_width=True):
        save_water(portion_size)
        st.rerun()
    
    if col2.button(f"🥤 {portion_size*2} мл", use_container_width=True):
        save_water(portion_size * 2)
        st.rerun()
    
    if col3.button(f"🍶 500 мл", use_container_width=True):
        save_water(500)
        st.rerun()
    
    if col4.button(f"🫙 1000 мл", use_container_width=True):
        save_water(1000)
        st.rerun()
    ...
    custom_amount = st.number_input("Количество (мл)", min_value=1, value=250, step=50)
